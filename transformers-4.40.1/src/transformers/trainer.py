...
class Trainer:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    for name, module in model.named_modules():
                        if hasattr(module, "step_placeholder"):
                            setattr(module, "step_placeholder", self.state.global_step)
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            logs["reg_freeze"] = round(torch.from_numpy(np.array(np.exp(-2.77*self.state.global_step/100000))).to(torch.bfloat16).item(), 3)
            logs["reg_learn0.01"] = round(2.0/(
                1.0 + np.exp(-0.01 / ((0.25 - 0.25 * self.state.global_step/100000) + 1e-4))
            ), 3)
