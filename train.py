...
def setup_everything():
    ...
    parser.add_argument("--padding_side", type=str, default="right", choices=["left", "right"], help="")
    parser.add_argument("--flash2", action="store_true", help="")
    parser.add_argument("--trust_remote_code", action="store_true", help="")
    parser.add_argument("--identity_loss", action="store_true", help="")
    ...
    training_args.output_dir = training_args.output_dir.replace(last_folder, formatted_time + "_" + last_folder)
    training_args.run_name = training_args.run_name.replace(last_folder, formatted_time + "_" + last_folder)
    training_args.logging_dir = training_args.logging_dir.replace(last_folder, formatted_time + "_" + last_folder)
    ...
    assert args.train_mode in ['full', 'lora', 'qlora', 'dora'], "task_type should be in ['full', 'lora', 'qlora']"
    assert sum([training_args.fp16, training_args.bf16]) == 1, "only one of fp16 and bf16 can be True"

    return args, training_args, cmd_args
...
def load_model(args, training_args, cmd_args):
    ...
    if args.train_mode == 'qlora':
        quantization_config = BitsAndBytesConfig(
            ...
            llm_int8_skip_modules=['dimdown0','dimdown1'],
        )
    else:
    ...
    # pruned_dict = torch.load("../LLM-Pruner/prune_log/llama_prune/pytorch_model.bin", map_location='cpu')
    # model = pruned_dict['model']
    # model.to(torch.bfloat16)
    model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, **model_kwargs)
    # import ipdb;ipdb.set_trace()
    for name, module in model.named_modules():
        if isinstance(module, DimDownLayer):
    ...
    if args.train_mode == 'full':
        ...
        peft_config = LoraConfig(
            ...
            bias="none",
            task_type="CAUSAL_LM",
            use_dora= (args.train_mode=='dora'),
        )

    # init peft model
    if args.train_mode in ['lora', 'qlora', 'dora'] and args.task_type in ['pretrain', 'sft']:
        model = get_peft_model(model, peft_config)
        for name, module in model.named_modules():
            if any(k in name for k in ["dimdown_in", "dimdown_out"]):
                # logger.info(name, "requires_grad = True")
                module.to(torch.float32)
                module.requires_grad_()
            if "dimdown_mask" in name and cmd_args.trainable_mask:
                module.to(torch.float32)
                module.requires_grad_()
            if "dimdown_scale" in name and cmd_args.identity_loss:
                module.to(torch.float32)
                module.requires_grad_()
...
def load_sft_dataset(args, tokenizer):
    ...
    if 'chatglm2' in args.model_name_or_path.lower():
        ...
        if args.train_file.endswith(".jsonl"):
            ...
            train_dataset = JsonSFTDataset(args.train_file, tokenizer, args.max_seq_length, template)
        else:
            if "combination_with_nomic" in args.train_file:
                train_dataset = IntuitionSFTDataset(args.train_file, tokenizer, args.max_seq_length, template)
            else:
                train_dataset = PandaSFTDataset(args.train_file, tokenizer, args.max_seq_length, template)
...
def init_components(args, training_args, cmd_args):
    ...
    if args.task_type == 'pretrain':
        ...
        data_collator = SFTDataCollator(tokenizer, args.max_seq_length, cmd_args.padding_side)
    else:
